...
populations_train <- c("Heron", "OffshoreCentral", "Pelorus", "Moore", "Lizard", "Global")
...
for (train_pop in populations_train) {
  for (test_pop in populations_test) {
    if (train_pop != test_pop) {
      ...
      rda_model <- readRDS(paste0("rda_models/rda_", train_pop, "_v2.rds"))
      ...
      # Prepare test population env data
      #X <- cbind(X[,1:3], scale(X[,4:9]))
